...
namespace VRKeys
{
    ...
    public class KeyboardController : MonoBehaviour
    {
        ...
        {
            keyboard.Enable();
            keyboard.SetPlaceholderMessage("Please input a password");
        ...
        private void Update()
        {
            if (PlayerMode.Instance.playerMode == PlayerMode.ePlayerMode.pc)
            {
                return;
            }
            if (TaskStepManagaer.Instance.IsEqualTaskId(4002) || Input.GetKeyDown(KeyCode.Space))
            {
        ...
        public void HandleSubmit(string text)
        {
            ...
            if (!text.Contains("JAYCEE LEE"))
            {
                keyboard.ShowValidationMessage("Password mistake");
        ...
        private IEnumerator SubmitPassword()
        {
            keyboard.ShowInfoMessage("Being verified, please wait... ;)");

            yield return new WaitForSeconds(2f);

            keyboard.ShowSuccessMessage("Verification success");
